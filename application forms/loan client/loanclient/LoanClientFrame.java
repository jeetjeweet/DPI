	...
	public LoanClientFrame() throws JMSException {
		...
		btnQueue.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				try {
					messageSenderGateway.send(messageSenderGateway.createObjectMessageLoanrequest(rr));
...
//				System.out.println("received message: "+omsg);
//				try {
//					RequestReply<LoanRequest,LoanReply> rr = (RequestReply<LoanRequest,LoanReply>) omsg.getObject();
//					for (int i = 0; i < listModel.size(); i++) {
////						if ((listModel.get(i).getRequest().getSsn() == rr.getRequest().getSsn())
////								&& (listModel.get(i).getRequest().getAmount() == rr.getRequest().getAmount())
////								&& (listModel.get(i).getRequest().getTime() == rr.getRequest().getTime()) && (listModel.get(i).getReply() == null)) {
////							listModel.setElementAt(rr, i);
////							break;
////						}
//						if(message.getJMSCorrelationID() == listModel.get(i).getRequest().getID()){
//							listModel.setElementAt(rr,i);
//							break;
//						}
//					}
...
//	}
	...
	private void setConnection() throws JMSException {
		...
		messageReceiverGateway = new MessageReceiverGateway("client");
   ...
   private RequestReply<LoanRequest,LoanReply> getRequestReply(LoanRequest request){    
	...
	//						}
					...
					if(omsg.getJMSCorrelationID() == listModel.get(i).getRequest().getID()){
						listModel.setElementAt(rr,i);
						break;
					}
